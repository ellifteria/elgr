...
type Triangle = {
    ...
    normalVertexA: Vertex;
    normalVertexB: Vertex;
    normalVertexC: Vertex;
}

function vertexLength(vertex: Vertex): number {
...
function scaleVertex(vertex: Vertex, scaleFactor: number): Vertex {
    return {x: vertex.x * scaleFactor, y: vertex.y * scaleFactor, z: vertex.z * scaleFactor};
}

function distanceBetweenVertices(vertex1: Vertex, vertex2: Vertex): number {
    return Math.sqrt(Math.pow(vertex1.x - vertex2.x, 2) + Math.pow(vertex1.y - vertex2.y, 2) + Math.pow(vertex1.z - vertex2.z, 2));
}

function calculateMidpoint(vertex1: Vertex, vertex2: Vertex): Vertex {
...
class TriangleMesh {
    ...
    addNormal(normal: Vertex): number {
        this.normals.push(normalizeVertex(normal));

        return this.normals.length - 1;
    }

    addTriangleFromVertices(vertex1: Vertex, vertex2: Vertex, vertex3: Vertex, normalVertexA: Vertex, normalVertexB: Vertex, normalVertexC: Vertex) {
    ...
    addTriangle(vertexIndex1: number, vertexIndex2: number, vertexIndex3: number, normalVertexIndex1: number, normalVertexIndex2: number, normalVertexIndex3: number) {
        if (
...
function createIcosahedron(radius: number = 1): TriangleMesh {
...
function createIcosphere(numRecursions: number, radius: number = 1): TriangleMesh {
...
function createRectangularPrism(width: number, height: number, depth: number): TriangleMesh {
